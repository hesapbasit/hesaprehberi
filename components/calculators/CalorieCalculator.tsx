...
export default function CalorieCalculator() {
  ...
  const result = useMemo(() => {
    ...
    return {
      ...
      bmr,
      maintenanceCalories,
      weightLossCalories: Math.max(maintenanceCalories - 500, 0),
      weightGainCalories: maintenanceCalories + 300,
    };
    ...
    <div className="rounded-3xl bg-white p-8 shadow-xl md:p-10">
      <div className="grid gap-10 lg:grid-cols-2">
        <div>
          ...
          <div className="mt-8 rounded-2xl border border-blue-100 bg-blue-50 p-5">
            <p className="text-sm leading-6 text-blue-800">
              Hesaplama, Mifflin-St Jeor formülü ve seçilen aktivite katsayısı
        ...
        <div className="rounded-3xl bg-gradient-to-br from-orange-500 to-red-600 p-8 text-white">
          ...
          <div className="mt-8">
            <p className="text-orange-100">Kilonuzu Korumak İçin</p>

            <h2 className="mt-3 text-4xl font-bold md:text-5xl">
              {result.valid
                ? formatNumber(result.maintenanceCalories)
                : 0}{" "}
              kcal
            </h2>
          </div>
          ...
          <div className="mt-10 grid gap-4 sm:grid-cols-2">
            ...
            <div className="rounded-2xl bg-white/10 p-6">
              ...
              <p className="mt-2 text-2xl font-bold">
                {result.valid
                  ? formatNumber(result.weightLossCalories)
                  : 0}{" "}
                kcal
              </p>
            </div>
          ...
          <div className="mt-6 rounded-2xl bg-white/10 p-6">
            <p className="text-sm text-orange-100">
              Kilo Almak İçin Tahmini Hedef
            </p>

            <p className="mt-2 text-2xl font-bold">
              {result.valid
                ? formatNumber(result.weightGainCalories)
                : 0}{" "}
              kcal
            </p>
          </div>
          ...
          <p className="mt-6 text-sm leading-6 text-orange-100">
            Sonuçlar genel tahmindir. Tıbbi durumlar, kas oranı ve günlük
            hareket miktarı gerçek ihtiyacı değiştirebilir.
          </p>
        </div>
      </div>
    </div>
